...
const initialForm: CoinSettingForm = {
  purchaseRate: "",
  currency: "",
...
export default function CoinSettingPage() {
  const [coinSettings, setCoinSettings] = useState<CoinSetting[]>([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [formData, setFormData] = useState<CoinSettingForm>(initialForm);
  ...
  const fetchCoinSettings = async () => {
    ...
    try {
      const res = await apiFetch("/api/coin-settings");

      if (!res.ok) {
        throw new Error("Failed to fetch coin settings");
      }

      const json = await res.json();

      setCoinSettings(
        (json.data || []).map((item: CoinSetting) => ({
          ...item,
          isActive: normalizeBool(item.isActive),
        })),
      );
    } catch (error) {
      console.error("Coin Setting fetch error:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(
        editId ? `/api/coin-settings/${editId}` : "/api/coin-settings",
        {
      ...
      alert(
        editId
          ? "Coin Setting updated successfully!"
          : "Coin Setting created successfully!",
      );

      resetForm();
      await fetchCoinSettings();
    } catch (error) {
      console.error("Coin Setting submit error:", error);
  ...
  // EDIT
  // =========================
  const handleEdit = (item: CoinSetting) => {
    setEditId(item.id);
  ...
  const handleDelete = async (id: string) => {
    const confirmed = confirm(
      "Are you sure you want to delete this Coin Setting?",
    );

    if (!confirmed) return;

    try {
      const res = await apiFetch(`/api/coin-settings/${id}`, {
        method: "DELETE",
      });
      ...
      alert("Coin Setting deleted successfully!");

      await fetchCoinSettings();
    } catch (error) {
      console.error("Coin Setting delete error:", error);
      alert("Something went wrong");
    }
  };
  ...
  // SEARCH
  // =========================
  const filteredCoinSettings = coinSettings.filter(
    (item) =>
      item.currency?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      String(item.purchaseRate).includes(searchQuery),
  );
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm md:text-base text-slate-600 mt-1">
                  Create and manage coin purchase settings.
                </p>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4 md:p-6">
              <div className="mb-6">
                <h2 className="text-lg md:text-xl font-semibold text-slate-900">
                  {editId ? "Edit Coin Setting" : "Coin Setting"}
                </h2>

                <p className="text-sm text-slate-600 mt-1">
                  Enter coin purchase setting details.
                </p>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4 md:p-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-6">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder="Search coin setting..."
              ...
              <div className="overflow-x-auto -mx-4 md:-mx-6">
                <div className="inline-block min-w-full align-middle px-4 md:px-6">
                          ...
                          <td colSpan={6}>
                            <div className="flex flex-col justify-center items-center h-48 gap-3">
                              ...
                              <span className="text-sm text-slate-500">
                                Loading Coin Settings...
                              </span>
                            </div>
                          </td>
                        </tr>
                      ) : (
                        filteredCoinSettings.map((item) => (
                  ...
                  {!loading && filteredCoinSettings.length === 0 && (
