...
export default function MerchSizePage() {
  ...
  const fetchCategories = async (): Promise<void> => {
    setLoading(true);
    try {
      const res = await fetch("/api/merch/category");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        res = await fetch(`/api/merch/category/${editId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
      } else {
        res = await fetch("/api/merch/category", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/merch/category/${id}`, {
        method: "DELETE",
      });
